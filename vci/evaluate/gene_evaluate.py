    ...
    for cov_category in cov_cats.keys():
        ...
        for pert_category in pert_cats.keys():
            ...
            if len(idx) > min_samples:
                ...
                mean_score_robust.append(r2_score(yt_m, yp_m))

                if de_idx.size > 0:
                    mean_score_de_mean.append(r2_score(yt_m[de_idx], yp_m[de_idx]))
                    mean_score_de_robust.append(r2_score(yt_m[de_idx], yp_m[de_idx]))

    return {
        "all_genes_r2_mean": np.mean(mean_score_mean),
        "all_genes_r2_robust": np.mean(mean_score_robust),
        "de_genes_r2_mean": np.mean(mean_score_de_mean),
        "de_genes_r2_robust": np.mean(mean_score_de_robust),
    }
